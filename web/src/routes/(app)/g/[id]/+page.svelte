<script lang="ts">
	...
	import { TagType, type Archive, type Task } from '$lib/models';
	...
	import GalleryCover from '$lib/components/gallery-cover.svelte';
		...
		const promise = new Promise<void>((resolve, reject) => {
			...
			try {
				...
				pMap(
					archive.images,
					async (image) => {
						const url = `${env.PUBLIC_CDN_URL}/image/${archive.key}/${image.filename}`;
...
<main class="container flex flex-col gap-2 md:flex-row">
	{#await data.archive}
		<div class="@container w-full flex-shrink-0 space-y-2 md:w-80">
		...
		<div class="@container w-full flex-shrink-0 space-y-2 md:w-80">
			<div class="w-full">
				<a href={`./${archive.id}/read/1/${$page.url.search}`}>
					<GalleryCover
						{archive}
						cover={archive.images[archive.thumbnail - 1]}
						src={`${env.PUBLIC_CDN_URL}/image/${archive.key}/${archive.thumbnail}/c`}
					/>
				</a>
			</div>
			...
			<div class="overflow-clip rounded shadow-md shadow-shadow">
				...
				{#if archive.released_at}
					<InfoSection name="Released">
						<p class="text-sm">{dateFormat(new Date(archive.released_at))}</p>
					</InfoSection>
				{/if}

				<InfoSection name="Added">
					<p class="text-sm">{dateTimeFormat(new Date(archive.created_at))}</p>
				</InfoSection>
			</div>
		</div>

		{#if archive.images.length}
			<GalleryThumbnails {archive} images={archive.images} />
		{/if}
	{/await}
</main>
