...
import { ImageFitMode, type Archive, type Image, type Tag, type Taxonomy } from '$lib/models';
...
export function getMetadata(archive: Archive) {
	return {
		...
		...(archive.released_at && { Released: new Date(archive.released_at).getTime() / 1000 }),
