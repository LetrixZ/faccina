<script lang="ts">
	...
	import { ImageFitMode, type Archive, type Image } from '$lib/models';
	...
	$: {
		...
		$nextPage = currentPage < archive.images.length ? currentPage + 1 : undefined;
	}
	...
	const changePage = (page?: number) => {
		...
		newImage.src = `${env.PUBLIC_CDN_URL}/image/${archive.key}/${imageInfo.filename}`;
	...
	const preloadImages = async () => {
		await pMap(
			...
			async (imageInfo) => {
				...
				const newImage = new Image(imageInfo.width, imageInfo.height);
				newImage.src = `${env.PUBLIC_CDN_URL}/image/${archive.key}/${filename}`;
...
<div class="flex h-dvh w-full flex-col overflow-clip">
	<div bind:this={container} class="relative my-auto flex h-full overflow-auto">
		...
		<img
			...
			src={`${env.PUBLIC_CDN_URL}/image/${archive.key}/${image?.filename}`}
			loading="eager"
