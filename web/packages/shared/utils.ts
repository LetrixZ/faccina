...
import { type Archive, type Image, type Tag, type Taxonomy } from "./models";
...
export function generateFilename(archive: Pick<Archive, "title" | "artists" | "circles" | "magazines">) {
...
export function getMetadata(archive: Archive) {
  return {
    ...
    Artist: archive.artists?.length ? archive.artists.map((artist) => artist.name)?.join(", ") : undefined,
    Groups: archive.circles?.length ? archive.circles.map((circle) => circle.name)?.join(", ") : undefined,
    Magazine: archive.magazines?.length ? archive.magazines.map((magazine) => magazine.name)?.join(", ") : undefined,
    Event: archive.events?.length ? archive.events.map((event) => event.name)?.join(", ") : undefined,
    Parody: archive.parodies?.length ? archive.parodies.map((parody) => parody.name)?.join(", ") : undefined,
    Publisher: archive.publishers?.length ? archive.publishers.map((publisher) => publisher.name).join(", ") : undefined,
    Pages: archive.pages,
